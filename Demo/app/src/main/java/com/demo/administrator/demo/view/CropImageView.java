...
public class CropImageView extends View {
	...
	//默认的裁剪图片宽度与高度
	private final int defaultCropWidth=800;
	private final int defaultCropHeight=500;
...
//Log.v("mTempDst:", "-------ration:"+(float)mDrawable.getBounds().width()/(float)mDrawable.getBounds().height());
	...
	protected void configureBounds()
	{
		if(isFrist)
		{
			...
			mDrawableFloat.set(0, floatTop,floatLeft+floatWidth, floatTop+floatHeight);
	...
	public Bitmap getCropImage()
	{
    	...
    	ret.recycle();
    	ret=newRet;
		Log.e("size","newRet"+newRet.getWidth() +"  "+newRet.getHeight());
		return ret;
	}
