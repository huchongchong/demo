...
public class LoadingView extends View {
    ...
    private float spinSpeed = 370.0f;
    ...
    /**
     * The constructor for the LoadingView
     */
    public LoadingView(Context context, AttributeSet attrs) {
        super(context, attrs);

        parseAttributes(context.obtainStyledAttributes(attrs, R.styleable.LoadingView));
    ...
    private void parseAttributes(TypedArray a) {
        ...
        circleRadius =
                (int) a.getDimension(R.styleable.LoadingView_matProg_circleRadius, circleRadius);

        fillRadius = a.getBoolean(R.styleable.LoadingView_matProg_fillRadius, false);

        barWidth = (int) a.getDimension(R.styleable.LoadingView_matProg_barWidth, barWidth);

        rimWidth = (int) a.getDimension(R.styleable.LoadingView_matProg_rimWidth, rimWidth);

        float baseSpinSpeed =
                a.getFloat(R.styleable.LoadingView_matProg_spinSpeed, spinSpeed / 360.0f);
        spinSpeed = baseSpinSpeed * 360;

        barSpinCycleTime =
                a.getInt(R.styleable.LoadingView_matProg_barSpinCycleTime, (int) barSpinCycleTime);

        barColor = a.getColor(R.styleable.LoadingView_matProg_barColor, barColor);

        rimColor = a.getColor(R.styleable.LoadingView_matProg_rimColor, rimColor);

        linearProgress = a.getBoolean(R.styleable.LoadingView_matProg_linearProgress, false);

        if (a.getBoolean(R.styleable.LoadingView_matProg_progressIndeterminate, false)) {
            spin();
        }
